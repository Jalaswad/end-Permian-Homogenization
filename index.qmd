...
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn paleobioDB conflicted piggyback
          CoordinateCleaner fossilbrush icosa tidyr tibble readr purrr downloadthis ggpubr")
...
```{r}
# Map visual.
plot(hexa, table(changhsingian_pbdb$cell), crs="ESRI:54009", 
     pal=c("#440154FF", "#2D708EFF", "#29AF7FFF", "#FDE725FF"), 
     breaks = c(0, 20, 200, 400, 800),
     reset=FALSE,main = "Changhsingian cell coverage")

plot(hexa, table(induan_pbdb$cell), crs="ESRI:54009", 
     pal=c("#440154FF", "#2D708EFF", "#29AF7FFF", "#FDE725FF"), 
     breaks = c(0, 20, 200, 400, 800),
     reset=FALSE,main = "Induan cell coverage")
```
...
The occurrence of a given taxa between distinct cells are evaluated against each other.
