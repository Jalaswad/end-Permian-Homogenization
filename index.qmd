...
The fossil occurrence data analysed in this study was retrieved from the [Paleobiology Database](https://paleobiodb.org/#/) on February of 2024. Data pre-processing made use of functions from the `fossilbrush` and `CoordinateCleaner` R packages.
...
# Set up the grid
hexa <-hexagrid(deg=5, sf=TRUE) #each deg = ~111 km
hexa
...
sumRes_10 |> 
  ...
  scale_size_continuous(breaks = c(5,10,15,20,25,30)) +
  geom_point(aes(size = n),shape = 21, fill = "white", stroke = 2) +
  scale_fill_manual(values =  c("#006633","slateblue3", "violetred")) + 
  scale_color_manual(values = c("#006633","slateblue3", "violetred")) + 
  labs(x = "Great Circle Distance (km)", 
       y = "Similarity Value",
