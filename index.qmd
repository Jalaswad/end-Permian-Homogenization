...
Jood Al Aswad<sup>1\*</sup>, Justin L. Penn<sup>2</sup>, Pedro Monarrez<sup>1</sup> <sup>3</sup>, Mohamad Bazzi<sup>1</sup>, Curtis Deutsch<sup>2</sup>, Jonathan L. Payne<sup>1</sup><br>
...
<sup>1</sup> Department of Earth and Planetary Sciences, Stanford University, 450 Jane Stanford Way, Stanford, CA 94305, USA.<br> <sup>2</sup> Department of Geosciences, Princeton University, Guyot Hall, Princeton, NJ 08544, USA.<br>
...
rpkg <- c("dplyr ggplot2 readr boot divvy terra divDyn paleobioDB conflicted piggyback
          CoordinateCleaner fossilbrush rgplates icosa tidyr tibble readr purrr downloadthis ggpubr")
...
The fossil occurrence data analysed in this study wa retrieved from the [Paleobiology Database](https://paleobiodb.org/#/) on February of 2024. Data pre-processing made use of functions from the `fossilbrush` and `CoordinateCleaner` R packages.

```{r message=FALSE}
# Read occurrence dataset.
pbdb <- 
  openxlsx::read.xlsx(xlsxFile = "/Users/geojood/Desktop/Homogenization/pbdb.xlsx") |> 
...
Next, visualize all occurrences for each stage, using the package `rgplates` on R. Please note that this version requires that you have the `Gplates` software installed in your computer, as it is the most optimal version of `rgplates`.

```{r}
# Map visualizations

# Get raw dataset before filtering for 20 occs
pbdb.2 <- read.csv("/Users/geojood/Desktop/Homogenization/pbdb.2.csv")
pbdb.2cha <- pbdb.2 |> filter(interval.ma==252.17)
pbdb.2ind <- pbdb.2 |> filter(interval.ma==251.2)
...
cells.cha <-locate(hexa,coords.cha) #to get no. of occupied cells
#str(cells.cha) #to see which cells have occ's
cells.ind <-locate(hexa,coords.ind)
#str(cells.cha)

tcells.cha <- table(cells.cha) 
#str(tcells.cha) 
...
Here, we visualize the grid we generated using the `icosa` and `rgplates` packages to display where cells and occurrences are located for each stage.
...
```{r}
# Changhsingian.
colnames(pbdb)[10] <- "lat"
colnames(pbdb)[11] <- "long" 
...
sumRes_03 |> 
  ...
  geom_errorbar(aes(ymin = pmax(avg-sdev), ymax=avg+sdev), width=0.05,linewidth=1) +
