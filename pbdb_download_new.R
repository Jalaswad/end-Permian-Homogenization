...
geoIntervals <- subset(geoIntervals, type %in% c("age"))
...
save(pbdb, genera, geoIntervals, file="pbdbAnimalsJun2024.Rdata")

#read in data files from Paleosize Database#
data.pbdb <- pbdb #PBDB data

write.csv(data.pbdb, file="pbdb.data.Aug2024.csv")
